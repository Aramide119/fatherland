...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.adverts.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="content">{{ trans('cruds.advert.fields.content') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.advert.fields.content_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="description">{{ trans('cruds.advert.fields.description') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.advert.fields.description_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="business_name">{{ trans('cruds.advert.fields.business_name') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.advert.fields.business_name_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label class="required" for="category">{{ trans('cruds.advert.fields.category') }}</label>
                <select class="form-control {{ $errors->has('advert_category_id') ? 'is-invalid' : '' }}" name="advert_category_id" id="category" required>
                    <option value disabled {{ old('advert_category_id', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach (App\Models\AdvertCategory::getCategorySelect() as $key => $value)
                        <option value="{{ $key }}">{{ $value }}</option>
                    @endforeach
                </select>
                @if($errors->has('advert_category_id'))
                    <div class="invalid-feedback">
                        {{ $errors->first('advert_category_id') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.advert.fields.category_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required">{{ trans('cruds.advert.fields.status') }}</label>
...
<script>
    ...
    init: function () {
@if(isset($advert) && $advert->content)
      var file = {!! json_encode($advert->content) !!}
