    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.contents.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('blog_content') ? 'is-invalid' : '' }}" name="blog_content" id="blog_content">{!! old('blog_content') !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('content_type') ? 'is-invalid' : '' }}" name="content_type_id" id="content_type_id" required>
                    @foreach($content_types as $id => $entry)
                        <option value="{{ $id }}" {{ old('content_type_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('content_category') ? 'is-invalid' : '' }}" name="content_category_id" id="content_category_id" required>
                    @foreach($content_categories as $id => $entry)
                        <option value="{{ $id }}" {{ old('content_category_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
