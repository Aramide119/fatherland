...
<div class="card">
    ...
    <div class="card-body">
     <form method="POST" action="{{ route('admin.products.product') }}" enctype="multipart/form-data" id="categoryForm">
        ...
        <form method="POST" action="{{ route("admin.products.update", [$product->id]) }}" enctype="multipart/form-data" id="product">
            @method('PUT')
            @csrf
          ...
          <div class="form-group">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description" required>{{ old('description', $product->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <div class="previewContainer">
                    ...
                    <img src="{{ $image->getUrl() }}" alt="Image" class="img-thumbnail m-2" style="max-width: 100px; max-height: 100px;">
                    @endforeach</div>
                @if($errors->has('photo'))
                    <div class="invalid-feedback">
                        {{ $errors->first('photo') }}
                ...
                <textarea class="form-control ckeditor {{ $errors->has('specification') ? 'is-invalid' : '' }}" name="specification" id="specification" required>{{ old('specification', $product->specification) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('delivery_amount') ? 'is-invalid' : '' }}" type="number" name="delivery_amount" id="delivery_amount" value="{{ old('delivery_amount', $product->delivery_amount) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sizes') ? 'is-invalid' : '' }}" name="sizes[]" id="sizes" multiple>
                    ...
                    <option value="{{ $size->id }}" {{ (in_array($size->id, old('sizes', [])) || $product->sizes->contains($id)) ? 'selected' : '' }}>{{ $size->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('colors') ? 'is-invalid' : '' }}" name="colors[]" id="colors" multiple>
                    ...
                    <option value="{{ $color->id }}" {{ (in_array($color->id, old('colors', [])) || $product->colors->contains($id)) ? 'selected' : '' }}>{{ $color->name }}</option>
                    @endforeach
                </select>
                @if($errors->has('subcategories'))
                    <div class="invalid-feedback">
