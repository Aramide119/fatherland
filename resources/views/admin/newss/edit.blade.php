...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.newss.update", $news->id ) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="news_content">{{ trans('cruds.news.fields.news_content') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('news_content') ? 'is-invalid' : '' }}" name="news_content" id="news_content">{!! old('news_content', $news->news_content) !!}</textarea>
                @if($errors->has('news_content'))
                    <div class="invalid-feedback">
                        {{ $errors->first('news_content') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.news.fields.news_content_helper') }}</span>
            </div>
            <div class="form-group">
