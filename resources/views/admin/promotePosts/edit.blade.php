...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.promotePosts.update", $promotedPost->id ) }}" enctype="multipart/form-data">
            @csrf
          
            <div class="form-group">
                <label class="required" for="amount">Amount</label>
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" value="{{ old('amount', $promotedPost->amount) }}" type="number" name="amount" id="amount" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('duration') ? 'is-invalid' : '' }}" placeholder="15 Days" type="text" name="duration" id="duration" value="{{ old('duration', $promotedPost->duration) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Models\News::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $promotedPost->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
